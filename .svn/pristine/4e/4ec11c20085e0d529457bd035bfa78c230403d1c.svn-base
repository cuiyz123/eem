...
import com.bps.client.common.help.json.parse.JSONArray;
...
public class EemDeductController extends BaseController {
    ...
    @ResponseBody
    public ModelAndView initReport(HttpServletRequest request, HttpServletResponse response,String nodeID) throws UIException {
        ...
        try {
            List<EemTempEntity> tempEntityList = eemCommonService.findTempList("deduct", getUserEntity(request),nodeID);
...
//            request.setAttribute("noticeList", eemNoticeService.findNoticeList(userEntity));
    ...
    //厂商与报表关系维护
    @RequestMapping(value = "/deductController.do", params = "method=initNewReport")
    @ResponseBody
    public ModelAndView initNewReport(HttpServletRequest request, HttpServletResponse response) throws UIException {
        UserEntity userEntity = getUserEntity(request);
       /* DeductExcelPage excelPage = deductService.initReport(userEntity, request, null);
        request.setAttribute("report", excelPage);*/
        try {
            //List<EemTempEntity> tempEntityList = eemCommonService.findTempList("sum", getUserEntity(request));
            //request.setAttribute("tempList",tempEntityList);
            request.setAttribute("yearStr", String.valueOf(Calendar.getInstance().get(Calendar.YEAR)));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return new ModelAndView(new InternalResourceView("/base/page/reportVenderTemp.jsp"));
    }


    @RequestMapping(value = "/deductController.do", params = "method=countVender")
    @ResponseBody
    public void countVender(HttpServletRequest request, HttpServletResponse response) throws UIException {
        String result = deductService.findAllVender();
